...
def check_yaml(file, suffix=('.yaml', '.yml')):
    ...
    return file if isinstance(file, (list, tuple)) else [file] if file.endswith(suffix) else []


def check_suffix(file, suffix=('.pt', '.onnx', '.tflite', '.pb', '.xml')):
    # Check if file has a valid suffix
    return file if isinstance(file, (list, tuple)) else [file] if file.endswith(suffix) else []


def check_img_size(img_size, s=32, floor=0):
    # Verify img_size is a multiple of stride s and greater than floor
